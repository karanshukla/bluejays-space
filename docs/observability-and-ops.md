...
Cheapest fix that doesn't require standing up a monitoring stack for a single-operator site: have `main()` POST a one-line summary (rows inserted, per-register counts, any source that returned zero candidates) to a webhook — a Slack incoming webhook or a Discord webhook are both a single `fetch` call, no new dependency. Land this as its own small change once classify has run for real in production for a bit and there's a sense of what's actually worth alerting on, rather than guessing upfront.
