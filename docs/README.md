...
1. **[frontend-roadmap.md](./frontend-roadmap.md)** — the biggest product gap: no per-headline permalinks, no Open Graph/social preview cards (including a concrete dynamic-OG-image design borrowed from `navyfragen-app`'s `opengraph-service`), no RSS, no sitemap — meaning there's currently no way to actually *share* an individual headline, which is the whole point of a FAX-Sports-style site. Also covers feed pagination and the visual/a11y items `ui-plan.md` left open.
