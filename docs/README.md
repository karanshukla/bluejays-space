...
1. **[frontend-roadmap.md](./frontend-roadmap.md)** — the biggest product gap: no per-headline permalinks, no Open Graph/social preview cards, no RSS, no sitemap — meaning there's currently no way to actually *share* an individual headline, which is the whole point of a FAX-Sports-style site. § 1 is now a fully decided, implementation-ready spec (route paths, exact meta tags, the Satori-based dynamic-OG-image render/cache design, RSS/sitemap/robots.txt) — an agent can build straight from it, no open product calls left. The favicon (§ 1.7), wide-screen scrapbook stretch (§ 2a), and admin redesign (§ 3) have since shipped. Still open: the permalinks/OG/RSS/sitemap work itself, feed pagination (decided: `?page=N`, 30/page), and the visual/a11y items `ui-plan.md` left open.
