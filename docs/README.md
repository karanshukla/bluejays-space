...
1. **[frontend-roadmap.md](./frontend-roadmap.md)** — the sharing/discovery foundation shipped: permalinks (`/h/{id}`), OG/Twitter meta tags, RSS feed, sitemap, `robots.txt`, and `SITE_URL` config are all live. The one remaining piece of § 1 is the dynamic per-headline OG image (§ 1.3 — Satori+resvg render, MinIO cache) — until it lands, shared permalinks unfurl with a static fallback image. Also covers feed pagination (decided: `?page=N`, 30/page, still open) and the visual/a11y items `ui-plan.md` left open.
