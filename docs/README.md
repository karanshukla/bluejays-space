...
1. **[frontend-roadmap.md](./frontend-roadmap.md)** — the biggest product gap: no per-headline permalinks, no Open Graph/social preview cards, no RSS, no sitemap — meaning there's currently no way to actually *share* an individual headline, which is the whole point of a FAX-Sports-style site. § 1 is now a fully decided, implementation-ready spec (route paths, exact meta tags, the Satori-based dynamic-OG-image render/cache design, RSS/sitemap/favicon/robots.txt) — an agent can build straight from it, no open product calls left. Also covers feed pagination (decided: `?page=N`, 30/page) and the visual/a11y items `ui-plan.md` left open.
