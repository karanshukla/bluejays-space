...
1. **Postgres** — Railway managed Postgres plugin. Grab its private `DATABASE_URL` (used by `web` and `ingest` only — `handles` doesn't touch it yet, see `docs/backend-api-plan.md` for why). Schema is applied automatically by `bluejays-web` on every boot — see "Schema application" below, nothing to do here by hand.
