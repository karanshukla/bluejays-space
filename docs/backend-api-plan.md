...
### 6. Handles: JSON file + GitHub PR, not Postgres — decided: keep it

`SPEC.md` used to say "DID stored in Postgres (same `bluejays-db` instance as the headlines table)." What's actually built (`handles/handles.go`, `handles/add-handle.go`) is meaningfully different and, arguably, better for a low-volume single-operator site: a submitted request opens a **GitHub PR** against `handles.json` (with rate limiting, dupe-DID warnings, and an async job-status spinner on the request page) rather than writing straight to a DB. Merging the PR *is* the human review gate — same philosophy as "no autonomous publishing" for headlines, just implemented as git review instead of an admin UI.

**Decided: keep the PR-flow design.** It already ships, works, and gives a free audit trail + rollback via git history. `SPEC.md`'s Handle Directory section has been updated to describe this as the final design, not the original Postgres plan — nothing left to reconcile there. Accepted tradeoffs, not gaps to close: `handles.json` lives in the same repo as the code, so a handle request requires `GITHUB_TOKEN` write access to this repo, and every deploy of `handles` re-reads the file at boot — no live update without a redeploy or Railway's git-based auto-redeploy on merge (confirm this is actually wired up — note the answer in `docs/production-verification.md`). The Postgres-migration alternative (a `handle_requests` table, its own `/admin` section, duplicated Cloudflare Access gating) is no longer on the table; don't resurrect it without a new explicit decision.

`README.md`'s former "once its DID storage moves off `handles.json`" line has been updated to match — see the Production (Railway) section.
