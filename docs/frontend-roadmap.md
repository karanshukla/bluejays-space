...
- `h/[id].astro` passes `title={headline.headline}`, `description={headline.subtitle}`, `ogType="article"`, `canonicalPath={/h/${id}}`. `ogImage` is not overridden yet (points at the static `/og-default.png`) — § 1.3 will wire the per-headline card here once it lands.
...
- **No crawler gate** in `h/[id].astro`: `og:image` always points at the dynamic route (`/api/og/{id}.png`), for every visitor, not just an allowlisted set of bot User-Agents. The original UA-sniffing design (below) turned out to be a maintenance trap — any unfurl bot not on the list (Pinterest, KakaoTalk, Viber, Mastodon, etc.) silently fell back to the generic site image — and bought nothing in return, since a human's browser never fetches an unrendered `<meta>` image and the route is already content-hash-cached and immutable.
...
**Trigger, superseded — was crawler-gated, now always-on**: the original design only pointed `og:image` at the dynamic route when the request's User-Agent matched a known link-preview bot (`Bluesky Cardyb`, `Discordbot`, `Twitterbot`, `Slackbot`, `facebookexternalhit`), on the theory that a normal browser visit didn't need the render. In practice this made every *other* social network's unfurl bot (Pinterest, KakaoTalk, Viber, Mastodon instances, etc.) fall back to the generic `/og-default.png` instead of the real per-headline card, and the gate saved nothing — a human's browser doesn't fetch an unrendered `<meta>` image regardless, and the image route was already publicly fetchable and cache-immutable. `h/[id].astro` now always sets `ogImage` to `/api/og/{id}.png`; the UA allowlist is gone.

**Caching, decided**: store generated PNGs in the existing MinIO bucket (already wired for photos, already has `web`'s `/api/images/*` proxy pattern to reuse) under key `og/{id}-{hash}.png`, where `hash` is a short SHA-256 (first 12 hex chars is plenty) of `` `${headline}|${subtitle}|${photo_ref}` ``. A content hash — not a bare TTL — makes invalidation exact: an admin edit changes the hash, so the old cached PNG is simply never looked up again (a stale orphan, cleaned up the same pass as `docs/backend-api-plan.md` item 3's general orphaned-image sweep — no separate cache-eviction mechanism needed). This sidesteps needing a new `updated_at` column on `headlines` purely for cache-busting.
