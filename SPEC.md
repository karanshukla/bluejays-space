...
| Bluesky | Free, open API | Recent posts/comments pulled as candidate source material for register 1 — specific posts (text + image), not just aggregate tone/flavor |
...
| Player photos | MLB/team editorial-use photos, Wikimedia Commons (public domain/CC), reused fan-created images from a sourced Reddit/Bluesky post (e.g. an existing meme edit), or screenshots of actual posts being riffed on | Must be rights-cleared for the use case; never AI-generated. Reused fan images keep a "Source: Reddit/Bluesky" credit, same as the platforms' own convention |
