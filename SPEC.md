...
- **Public feed pagination**: `/` renders the first page server-side (SEO- and no-JS-friendly) and infinite-scrolls the rest via an `IntersectionObserver` that fetches `/feed-fragment` — an HTML fragment rendered through the same `HeadlineCard` as the first page, so there is exactly one card template in the codebase. Pages are keyset-paginated on `(published_at, id)` (cursor = oldest row shown), which stays stable as new headlines publish between page loads where OFFSET would drift. The permalink backdrop (`/h/{id}`) is likewise capped to the first page rather than loading the full history.
