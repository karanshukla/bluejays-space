...
- **Not verified live**: whether any of the above is actually deployed to Railway with a working Cloudflare Access application — see `docs/launch-checklist.md`.
- **Next up**: sharing/SEO (no per-headline permalinks, OG tags, RSS, or sitemap yet), CRUD gaps (no unpublish/discard route), ops visibility for the ingest cron — see `docs/README.md` → Roadmap for the full list.
